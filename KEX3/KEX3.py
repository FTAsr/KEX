# -*- coding: utf-8 -*-
"""
Created on Thu Aug 10 11:32:53 2017
...
class KEX3:
    ...
    def pre_stripper(self, seperated):
    ...
    a set of words
    '''
    def candidate_word_extractor(self, stopwords, seperated):
    ...
    '''
    def candidate_phrase_extraction(self, stopwords, seperated):
        self.stopwords = stopwords
        self.seperated = seperated
        phrases = []
        current_word = ''
    ...
    '''
    def phrase_filtering(self, candidate_phrases, thresholder, diction, random_vect):
        self.random_vect = random_vect 
    ...
    '''
    def lemmatization(self, clean_phrases):
        self.clean_phrases = clean_phrases
    ...
    '''
    def distrib_represent_conversion(self, clean_phrases, diction, random_vect):
        self.clean_phrases = clean_phrases
        self.diction = diction
        self.random_vect = random_vect
        mapper = dict()
    ...
    '''
    def clustering(self, mapper, k_count):
        self.mapper = mapper
        ...
        for m in mapper:
            current = mapper[m]
            li = (self.cosine_broad(current, np.array(mapper.values())))    
    ...
    '''
    def convert_list_to_dict(self, lister):
        self.lister = lister
        chain_dict = dict()
        for c in lister:
            chain_dict[c[0]] = c[1:]
    ...
    a set of clusters.
    '''
    def chain_scoring(self, chains, mapper):
        self.mapper = mapper
        self.chains = chains
        if type(chains) != list:
            chains = self.convert_dict_to_list(chains)
        chain_scores= dict()
    ...
    a set of clusters.
    '''
    def skip_agglomerative_distance(self, clusters, mapper):
        self.mapper = mapper
        self.clusters = clusters
        if type(clusters) != list:
            clusters = self.convert_dict_to_list(clusters)
        dists = dict()  
    ...
    '''
    def tfidf_score(self, di, lemmatized, no_of_keywords):
        self.di = di
    ...
    '''
    def skip_agglomerative_method(self, mapper, count):
        self.mapper = mapper
        self.count = count
        items = mapper.keys()
        dist_dict = {}
        ...
        for i in range(0, len(items)):
            ...
            first = self.cosine_broad(mapper[center], values)
            first[first == 0] = 100.0
            ...
            while len(words) < count:
                ...
                next_word_dist = self.cosine_broad(agg, values)
    ...
    '''
    def chain_length(self, all_clusters, mapper):
        self.mapper = mapper        
        self.all_clusters = all_clusters
        chain_scores = self.chain_scoring(all_clusters, mapper)
    ...
    '''
    def find_closest_word(self, word, mapper, diction):
        self.word = word
        self.mapper = mapper
        self.diction = diction
        ...
        distances = self.cosine_broad(word_vector, np.array(mapper.values()))
    ...
    '''
    def average_diameter(self, clusters, mapper):
        self.mapper = mapper
        self.clusters = clusters
        distance_dict = {}
        for c in clusters:
            distance_dict[c] = np.mean(self.cosine_broad(mapper[c], np.array(mapper.values())))
        #Find min distance
    ...
    '''
    def metric_chooser(self, clusters, distance_metric, mapper):
        self.clusters = clusters
        self.distance_metric = distance_metric
        self.mapper = mapper
        if distance_metric == "avg":
            cluster_center = self.average_diameter(clusters, mapper)
            pre_tf_scored_words = [cluster_center] + list(clusters[cluster_center])
        if distance_metric == "chain":
            pre_tf_scored_words = self.chain_length(clusters, mapper)
        if distance_metric == "skip":
            cluster_center = self.skip_agglomerative_distance(clusters, mapper)
            pre_tf_scored_words = [cluster_center] + list(clusters[cluster_center])
    ...
    '''
    def relative_keywords(self, text, stopwords):
        self.text = text
        self.stopwords = stopwords
        sepe = self.pre_stripper(text.split())
        candidate_words = self.candidate_word_extractor(stopwords, sepe)
        lemmatized = self.stripper(self.lemmatization(candidate_words))
        return lemmatized
    ...
    used to preprocess the text. 
    '''
    def absolute_keywords(self, text, stopwords, diction):
        self.text = text
        self.stopwords = stopwords
        self.diction = diction
        random_vect = np.random.rand(300)
        sepe = self.pre_stripper(text.split())
        candidate_words = self.candidate_phrase_extraction(stopwords, sepe)
        candidate_words = self.stripper(candidate_words)
        phrases = self.phrase_filtering(candidate_words, 0.85, diction, random_vect)
        lemmatized = self.stripper(self.lemmatization(phrases))
        return lemmatized

class Extractor():
    
    def __init__(self, stopwords, diction, di):
        self.stopwords = stopwords
        self.diction = diction
        self.di = di
        return None
    ...
    def clustering_keywords(self, text, distance_metric):
        self.text = text
        self.distance_metric = distance_metric
        #self.diction = diction
        #self.di = di
        k = KEX3()
        lemmatized = k.relative_keywords(text, self.stopwords)
        mapper = k.distrib_represent_conversion(lemmatized, self.diction, np.random.rand(300))
        no_of_keywords = int(np.floor(len(lemmatized)/1.5))
        count = len(lemmatized)/2
    
        clusters = k.clustering(mapper, no_of_keywords)
        pre_tf_scored_words = k.metric_chooser(clusters, distance_metric, mapper)
        tf_scored_words = k.tfidf_score(self.di, pre_tf_scored_words, count)
        
        return tf_scored_words
    
    def skip_agglomeration_keywords(self, text, distance_metric):
        self.text = text
        self.distance_metric = distance_metric
        k = KEX3()
        lemmatized = k.relative_keywords(text, self.stopwords)
        ...
        clusters = k.skip_agglomerative_method(mapper, count)
        pre_tf_scored_words = k.metric_chooser(clusters, distance_metric, mapper)
        return pre_tf_scored_words
    
    def clustering_keyphrases(self, text, distance_metric, no_of_keywords):
        self.text = text
        self.distance_metric = distance_metric
        self.no_of_keywords = no_of_keywords
        k = KEX3()
        lemmatized = k.absolute_keywords(text, self.stopwords, self.diction)
        mapper = k.distrib_represent_conversion(lemmatized, self.diction, np.random.rand(300))
        clusters = k.clustering(mapper, no_of_keywords)
        pre_tf_scored_words = k.metric_chooser(clusters, distance_metric, mapper)
        return pre_tf_scored_words
    
    def skip_agglomeration_keyphrases(self, text, distance_metric, no_of_keywords):
        self.text = text
        self.distance_metric = distance_metric
        self.no_of_keywords = no_of_keywords
        k = KEX3()
        lemmatized = k.absolute_keywords(text, self.stopwords, self.diction)
        mapper = k.distrib_represent_conversion(lemmatized, self.diction, np.random.rand(300))
        clusters = k.skip_agglomerative_method(mapper, no_of_keywords)
        pre_tf_scored_words = k.metric_chooser(clusters, distance_metric, mapper)
        return pre_tf_scored_words
    
    def clustering_with_related_word(self, text, no_of_keywords, related_word):
        self.text = text
        self.no_of_keywords = no_of_keywords
        self.relate_word = related_word
        k = KEX3()
        lemmatized = k.absolute_keywords(text, self.stopwords, self.diction)
        mapper = k.distrib_represent_conversion(lemmatized, self.diction, np.random.rand(300))
        clusters = k.clustering(mapper, no_of_keywords+1)
        
        if related_word not in clusters:
            closest_word = k.find_closest_word(related_word, mapper, self.diction)
            return [closest_word] + list(clusters[closest_word])        
        if related_word in clusters:
            return [related_word] + list(clusters[related_word])

    def skip_agglomeration_with_related_word(self, text, no_of_keywords, related_word):
        self.text = text
        self.no_of_keywords = no_of_keywords
        self.relate_word = related_word
        k = KEX3()
        lemmatized = k.absolute_keywords(text, self.stopwords, self.diction)
        mapper = k.distrib_represent_conversion(lemmatized, self.diction, np.random.rand(300))
        clusters = k.skip_agglomerative_method(mapper, no_of_keywords+1)
